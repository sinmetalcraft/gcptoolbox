...
import (
	...
	"github.com/sinmetalcraft/gcptoolbox/internal/times"
)
...
type SpannerToAvroOnGCSJobRequest struct {
	...
	OutputDirAddCurrentDateJST bool `json:"outputDirAddCurrentDateJST"`
	...
	AvroTempDirectory string `json:"avroTempDirectory"`
	...
	SnapshotTimeJSTDayChangeTime bool `json:"snapshotTimeJSTDayChangeTime"`
	...
	SpannerDatabaseID string `json:"spannerDatabaseId"`
	...
	ShouldExportTimestampAsLogicalType bool `json:"shouldExportTimestampAsLogicalType"`
	...
	ShouldExportRelatedTables bool `json:"shouldExportRelatedTables"`

	// Spanner 呼び出しのリクエストの優先度。
	// 有効な値は HIGH、MEDIUM、LOW です。デフォルト値は MEDIUM です。
	SpannerPriority string `json:"spannerPriority"`
	...
	DataBoostEnabled bool `json:"dataBoostEnabled"`
